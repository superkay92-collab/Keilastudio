...
function mapProduct(p: (typeof libProducts)[0]) {
  const tiers = (p as (typeof libProducts)[0] & { lengths?: Array<{ inches: number; fallsAt?: string; price: number }> }).lengths;
  const hasTiers = Array.isArray(tiers) && tiers.length > 0;
  const startingPrice = hasTiers ? Math.min(...tiers!.map((t) => t.price)) : p.price;
  return {
    id: p.id,
    name: p.name,
    ...
    <div className="rounded-xl overflow-hidden flex flex-col card-hover" style={{ background: C.white, border: `1px solid ${C.oliveSoft}44` }}>
      ...
      <div className="p-4 flex flex-col flex-1 items-center text-center">
        ...
        <div className="flex flex-col items-center gap-2 mt-3 w-full">
          <span className="f-mono" style={{ color: C.oliveDeep, fontSize: 15, fontWeight: 500 }}>
            {product.hasTiers && <span style={{ fontSize: 10, opacity: 0.6, marginRight: 4 }}>FROM</span>}
            {cedis(product.startingPrice)}
          </span>
            ...
            {product.inStock && (
              product.hasTiers ? (
                <a href={`/products/${product.id}`}
                  className="f-body px-3 py-1.5 rounded-full inline-block" style={{ background: C.oliveDeep, color: C.cream, fontSize: 12.5, fontWeight: 600, textDecoration: "none" }}>
                  Choose Length
                </a>
              ) : (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-0">
          ...
          <div className="px-5 py-5 overflow-y-auto">
            ...
            <div className="mt-5 pt-4" style={{ borderTop: `1px solid ${C.oliveSoft}44` }}>
              <span className="f-mono" style={{ fontSize: 22, color: C.oliveDeep, fontWeight: 500 }}>
                {product.hasTiers && <span style={{ fontSize: 11, opacity: 0.6, marginRight: 6 }}>FROM</span>}
                {cedis(product.startingPrice)}
              </span>
              {!product.inStock ? (
                ...
                <div className="mt-3 flex gap-2">
                  {product.hasTiers ? (
                    <a href={`/products/${product.id}`}
                      className="f-body flex-1 py-3 rounded-full text-center inline-block" style={{ background: C.oliveDeep, color: C.cream, fontSize: 13.5, fontWeight: 600, textDecoration: "none" }}>
                      Choose Length
                    </a>
                  ) : (
